...
class GpioHandler {
  constructor (dpmcModule, options = {}, configKeys = {}) {
    ...
    this._rememberLastStatusKey = configKeys.rememberLastStatus

    this._defaultRememberLastStatus = options.rememberLastStatus || false
  ...
  async init () {
    ...
    this._rememberLastStatus = (this._rememberLastStatusKey && await this._module.config.get(this._rememberLastStatusKey)) || this._defaultRememberLastStatus
    this._gpioNumber = await this._module.config.get(this._gpioKey)

    this._storageKey = `gpio_${this._gpioNumber}_state`

    if (this._rememberLastStatus) {
      try {
